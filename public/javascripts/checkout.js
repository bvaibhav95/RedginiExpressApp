...
$("#addrow").on("click", function() {
  ...
  cols +=
    '<td><input id="removeCake" type="button" class="ibtnDel btn btn-sm" value="X"></td>';
  newRow.append(cols);
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        for (var i = 0; i < data.availableWeights.length; i++) {
          $("#available_cake_weight").append(
            '<option value="' +
              data.availableWeights[i] +
              '">' +
              data.availableWeights[i] +
              "</option>"
          );
  ...
  $.ajax({
    ...
    success: function(data) {
      if (!isNaN($("#available_cake_weight").val())) {
        $("#cake_price").val(
          "Rs." +
            data.halfKgPrice * $("#available_cake_weight").val() * 2 +
            "/-"
        );
      } else {
        $("#cake_price").val("Rs.X");
